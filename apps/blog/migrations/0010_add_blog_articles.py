...
def add_blog_articles(apps, schema_editor):
    ...
    #это колхозный способ, при созд модели мы должны обязательно описать все поля, смотреть model.py class Article
